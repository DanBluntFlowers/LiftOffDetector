...
class Report:
    ...
    def get_company_list(language_setting='fr'):
        ...
        if language_setting in ['fr', 'all']:
            ...
            list_of_companies = fast_winners.get_list(language_setting=language_setting, double_click = True)
    ...
    def store_report(self, story_packs):
        ...
        addendum = str(datetime.today()).replace(' ', '_').replace(':', '-').split('.')[0]
        old_path = f'Reports\\full_report_archive_{addendum}.csv'

        try:
            full_report = pd.read_csv(path)
            full_report.to_csv(old_path, sep=';', decimal=',', encoding='utf-8-sig', index=False)
            full_report = full_report.append(df)
            full_report = full_report.drop_duplicates()
            full_report.to_csv(path, sep=';', decimal=',', encoding='utf-8-sig', index=False)
